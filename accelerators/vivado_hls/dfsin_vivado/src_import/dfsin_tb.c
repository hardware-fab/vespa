...
main ()
{
  ...
  int i;
      ...
      for (i = 0; i < N; i++)
	{
	  ...
	  float64_sin_hw(&test_in[i], &result_dut);
    float64_sin_sw(&test_in[i], &result_sw);

	  main_result += (result_dut != test_out[i]);

	  printf
	    ("input=%016llx expected=%016llx output_hw=%016llx output_sw=%016llx (%lf)\n",
	     test_in[i], test_out[i], result_dut, result_sw, ullong_to_double (result_dut));
	}
      printf ("%d\n", main_result);
      return main_result;
    }
