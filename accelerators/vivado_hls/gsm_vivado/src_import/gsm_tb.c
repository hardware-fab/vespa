...
{
  int main_result;
  word so_dut[N], so_sw[N];
  word LARc_dut[M], LARc_sw[M];
      main_result = 0;
	...
	for(i=0; i<N; i++)
	{
     ...
     gsm_main_hw(so_dut, LARc_dut);
     gsm_main_sw(so_sw, LARc_sw);


      for (i = 0; i < N; i++)
      {
    	  main_result += (so_dut[i] != so_sw[i]);
    	  printf("SO[%d]:    Expected:%d  -  Output_hw:%d  -  Output_sw:%d\n", i, (int)outData[i], (int)so_dut[i], (int)so_sw[i]);
      }
	for (i = 0; i < M; i++)
	{
		main_result += (LARc_dut[i] != LARc_sw[i]);
		printf("LARc[%d]:    Expected:%d  -  Output_hw:%d  -  Output_sw:%d\n", i, (int)outLARc[i], (int)LARc_dut[i], (int)LARc_sw[i]);
	}
      printf ("%d\n", main_result);
      return main_result;
}
