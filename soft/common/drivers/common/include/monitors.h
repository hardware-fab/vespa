...
#define ESP_MON_READ_NOC_EJECTS 7           //GM change: my new metric (incremented the values of the following ones)
#define ESP_MON_READ_NOC_QUEUE_FULL_TILE 8	 //requires tile_index
#define ESP_MON_READ_NOC_QUEUE_FULL_PLANE 9  //requires noc_index
//#define ESP_MON_READ_TRANSIT 10              //GM change: my new transit registers
...
#define MON_ACC_ROUNDTRIP_TIME			18  //GM change
...
#define MON_NOC_TILE_EJECT_BASE_INDEX	(MON_NOC_TILE_INJECT_BASE_INDEX + NOCS_NUM) //28  //GM change: added the ejection (incremented the following one)
#define MON_NOC_QUEUES_FULL_BASE_INDEX	(MON_NOC_TILE_EJECT_BASE_INDEX + NOCS_NUM) //34
//#define MON_TRANSIT_TILE_INJECT_BASE_INDEX (MON_NOC_QUEUES_FULL_BASE_INDEX + NOCS_NUM*NOC_QUEUES) //64  //GM change: added the transit registers
//#define MON_TRANSIT_TILE_EJECT_BASE_INDEX (MON_TRANSIT_TILE_INJECT_BASE_INDEX + SOC_NTILES*NOCS_NUM)   //GM change: added the transit registers
...
typedef struct acc_stats_t {
	...
	unsigned int acc_roundtrip_time; //GM change
} esp_acc_stats_t;
...
typedef struct esp_monitor_vals {
	...
	unsigned int noc_ejects[SOC_NTILES][NOC_PLANES];  //GM change: added the ejects
	unsigned int noc_queue_full[SOC_NTILES][NOC_PLANES][NOC_QUEUES];
	//unsigned int transit_injects[SOC_NTILES][NOC_PLANES];  //GM change: added transit regs
	//unsigned int transit_ejects[SOC_NTILES][NOC_PLANES];   //GM change: added transit regs
} esp_monitor_vals_t;
