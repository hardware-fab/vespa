...
class NoCFrame(Pmw.ScrolledFrame):
  ...
  def create_tile(self, frame, tile):
    ...
    #field to insert the parallelism level
  ...
  def __init__(self, soc, bottom_frame):
    ...
    #adding an entry for the number of NoC planes
    ...
    #adding an entry for the size of the main memory
    ...
    #adding an entry for the size of each additional memory
  ...
  def update_msg(self):
    ...
    #verify that the parallelism level of each accelerator is a power of two
    n_cores_string = ""
    n_cores_ok = True
    for y in range(0, self.noc.rows):
      for x in range(0, self.noc.cols):
    ...
    if (tot_cpu > 0) and \
      ...
      #removing the limit on three memories. Does it work?
      #if (tot_mem == 3):
