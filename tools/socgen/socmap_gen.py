...
#  my memories are way smaller than 2GB
...
class tile_info:
  ...
  has_pll = 0
  has_clkbuf = 0
  # among tiles info I add the clock domain to which the noc belongs
  noc_clk_region = 0;
  ...
  def __init__(self, soc):
    ...
    for x in range(soc.noc.rows):
      for y in range(soc.noc.cols):
        ...
        # initialize the noc clock region
        self.tiles[t].noc_clk_region = soc.noc_clk_region[x+y*soc.noc.cols]
...
def print_global_constants(fp, soc):
  ...
  # I really need the number of NoC as a global parameter
...
def print_constants(fp, soc, esp_config):
  ...
  fp.write("  constant CLOCK_DOMAINS_NUM : integer := " + str(esp_config.ndomain) + ";  -- I need this info as a global variable due to package dependence issues\n\n");
  #
...
def print_mapping(fp, soc, esp_config):
  ...
  for i in range(0, esp_config.nmem):
    #fp.write("    " + str(i) + " => 16#" + format(mask, '03X') + "#,\n")
    # a specific mask for the memory
    #(Note: if secondary memory is smaller than 1MB, we make the SoC believe that its size is 1MB to avoid issues)
    local_mask = 0
    if i == 0:
  ...
  for i in range(esp_config.nacc):
    ...
    # adding the number of cores for accelerator parallelism
    ...
    # since the interrupt max number is limiting the number of accelerators I can have, I remove the interrupts on accelerators altogether
  ...
  #
  fp.write("  --  Get accelerator parallelism level from tile ID\n")
  ...
  #
  fp.write("  --  Get accelerator parallelism level from accelerator ID\n")
  ...
  #
  fp.write("  --  Get log2 of accelerator parallelism level from tile ID\n")
  ...
  #
  fp.write("  --  Get log2 of accelerator parallelism level from accelerator ID\n")
  ...
  # parameter for the noc domain
  fp.write("  --  get noc clock domain from tile ID\n")
  ...
  #  this is an extra parameter for the frequency of a certain clock domain (not needed anymore)
  fp.write("  --  get frequency from domain ID\n")
  ...
  for i in range(0, esp_config.ntiles):
    # the domain master of a memory tile is the tile itself (that is, the icclk)
  ...
  # get the domain master ID of a noc
  fp.write("  --  get tile ID of the DVFS domain master from the noc clock region\n")
  fp.write("  constant noc_domain_master : attribute_vector(0 to CFG_TILES_NUM - 1) := (\n")
  for i in range(0, esp_config.ntiles):
    # the master of region zero is the memory tile
