...
def write_ap_acc_signals(f):
  f.write("\n")
  f.write("-- all signals have been modified to allow for parallel execution\n")
  f.write("\n")
  f.write("--Constants\n")
  f.write("constant N_CORES : integer := tile_n_cores(this_tile_id);\n")
  f.write("constant LOG2_N_CORES : integer := tile_log2_n_cores(this_tile_id);\n")
  f.write("\n")
  ...
  if is_catapulthls_cxx_if:
    ...
    for param in acc.param:
      if not param.readonly:
        ...
        # this is the batch length, and needs to be treated differently
  ...
  with open(template_dir + '/genacc.vhd', 'r') as ftemplate:
    for tline in ftemplate:
      ...
      for acc in accelerator_list:
        ...
        f.write("      this_tile_id       : integer range 0 to CFG_TILES_NUM := 0 \n")
        f.write("    );\n")
        f.write("\n")
        f.write("    port (\n")
    ...
    for tline in ftemplate:
      ...
      for acc in accelerator_list:
        if acc.hls_tool == 'catapult_hls_cxx':
          ...
          f.write("use work.sld_devices.all;\n")
          f.write("use work.allacc.all;\n\n")
          f.write("-- libraries for parallel execution\n")
          ...
          for impl in acc.hlscfg:
            ...
            # right now, parallel execution has been tested only for vivado hls-generated accelerators
  ...
  with open(template_dir + '/sldacc.vhd', 'r') as ftemplate:
    for tline in ftemplate:
      ...
      for acc in accelerator_list + axi_accelerator_list:
        ...
        f.write("    generic (\n")
        f.write("      SIMULATION     : boolean := false;  -- need this bool for pll library mismatch\n")
        f.write("      hls_conf       : hlscfg_t;\n")
        ...
        f.write("      pll_clk_freq   : integer range 0 to 10 := 0; -- clock frequency that must be returned from the pll (1=max, 10=min)\n")
        f.write("      this_tile_id   : integer range 0 to CFG_TILES_NUM := 0 \n")
        f.write("    );\n")
        f.write("\n")
        f.write("    port (\n")
        f.write("      rst               : in  std_ulogic;\n")
        f.write("      clk               : in  std_ulogic;\n")
        f.write("      base_rst          : in    std_logic;  -- a reset for clocking resources\n")
        f.write("      refclk            : in  std_ulogic;\n")
        f.write("      pllbypass         : in  std_ulogic;\n")
        f.write("      pllclk            : out std_ulogic;\n")
        f.write("      pll_locked        : out std_logic;  -- bringing internal lock to the top module\n")
        ...
        f.write("      freq_data_in      : in std_logic_vector(GM_FREQ_DW-1 downto 0);  -- input freq data\n")
        f.write("      freq_empty_in     : in std_logic);  -- freq data empty\n")
        f.write("  end component;\n\n")
        f.write("\n")
  f.close()
  ftemplate.close()
    ...
    for tline in ftemplate:
      if tline.find("-- <<entity>>") >= 0:
        ...
        if is_axi:
          ...
          f.write("      this_tile_id  => this_tile_id \n")
          f.write("    )\n")
    ...
    for tline in ftemplate:
      if tline.find("-- <<accelerator-wrappers-gen>>") >= 0:
        for acc in accelerator_list + axi_accelerator_list:
          ...
          f.write("      generic map (\n")
          f.write("        SIMULATION     => SIMULATION,  -- need this bool for pll library mismatch\n")
          ...
          f.write("        pll_clk_freq   => pll_clk_freq, -- clock frequency that must be returned from the pll (1=max, 10=min)\n")
          f.write("        this_tile_id   => this_tile_id) \n")
          f.write("      port map (\n")
          f.write("        rst               => tile_rst,  -- this is actually the local reset\n")
          f.write("        clk               => clk_feedthru,\n")
          f.write("        base_rst          => base_rst,  -- a reset for clocking resources\n")
          f.write("        refclk            => dvfs_clk,\n")
          f.write("        pllbypass         => pllbypass,\n")
          f.write("        pllclk            => clk_feedthru,\n")
          f.write("        pll_locked        => pll_locked,  -- bringing internal lock to the top module\n")
          ...
          f.write("        freq_data_in      => freq_data_in,  -- input freq data\n")
          f.write("        freq_empty_in     => freq_empty_in  -- freq data empty\n")
          f.write("      );\n")
