...
use work.socmap.all; -- I need this library for roundtrip monitoring
...
-- <<architecture>>
  ...
  -- custom monitor for the roundtrip time
...
begin
  ...
  -- a monitor for the roundtrip time
  rtt_mon_inst : roundtrip_time_monitor
  generic map (
    DATA_WIDTH    => MON_ACC_RTT_SIZE,
    LOG2_N_CORES  => tile_log2_n_cores(this_tile_id)
  )
  port map (
    clk_i                   => clk,
    rstn_i                  => rst,
